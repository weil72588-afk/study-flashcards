...
function showScreen(id){$$('.screen').forEach(x=>x.classList.remove('active'));$('#'+id).classList.add('active');$('#addCardFab').style.display=id==='studyScreen'?'none':'block';window.scrollTo(0,0)}
...
async function renderAll(){
  ...
  renderDecks();renderDeckSelects();if($('#browseScreen').classList.contains('active'))renderBrowse();
}
function renderDecks(){const el=$('#deckList');if(!decks.length){el.innerHTML='<div class="empty">还没有卡组。先新建一个卡组，再添加第一张闪卡。</div>';return}el.innerHTML=decks.map(d=>{const cs=cards.filter(c=>c.deckId===d.id),due=cs.filter(isDue).length,weak=cs.filter(isWeak).length;return `<div class="deck"><div class="deck-main"><div class="deck-name">${esc(d.name)}</div><div class="deck-meta">${cs.length} 张 · 今日到期 ${due}${weak?` · 模糊 ${weak}`:''}</div></div><div class="deck-actions"><button class="mini" data-study-deck="${d.id}">复习</button><button class="mini" data-edit-deck="${d.id}">•••</button></div></div>`}).join('')}
...
function renderBrowse(){const q=$('#searchInput').value.trim().toLowerCase(),deckId=$('#deckFilter').value;let arr=cards.filter(c=>(!deckId||c.deckId===deckId)&&(!q||[c.front,c.back,c.note,c.source,...(c.tags||[])].join(' ').toLowerCase().includes(q)));arr.sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt));$('#browseList').innerHTML=arr.length?arr.map(c=>`<div class="card-row"><div class="card-row-main"><div class="card-front">${esc(c.front||'（图片卡）')}</div><div class="card-sub"><span class="badge">${esc(cardDeckName(c))}</span>${isWeak(c)?'<span class="badge">模糊</span>':''}${c.source?` · ${esc(c.source)}`:''}</div></div><button class="row-menu" data-edit-card="${c.id}">编辑</button></div>`).join(''):'<div class="empty">没有匹配的卡片。</div>'}
...
document.addEventListener('click',e=>{const sd=e.target.closest('[data-study-deck]');if(sd)buildTodaySession(sd.dataset.studyDeck);const ed=e.target.closest('[data-edit-deck]');if(ed)openDeckModal(ed.dataset.editDeck);const ec=e.target.closest('[data-edit-card]');if(ec)openCardModal(ec.dataset.editCard)});
